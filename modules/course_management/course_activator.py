...
async def activate_course(interaction: nextcord.Interaction, channel: nextcord.TextChannel):
	"""Move a course from the inactive courses category to the active one."""
	await __move_course(ctx, channel, active=True)


async def deactivate_course(interaction: nextcord.Interaction, channel: nextcord.TextChannel):
	"""Move a course from the active courses category to the inactive one."""
	await __move_course(ctx, channel, active=False)


async def deactivate_all_courses(interaction: nextcord.Interaction):
...
async def __move_course(interaction: nextcord.Interaction, channel: nextcord.TextChannel, active: bool):
