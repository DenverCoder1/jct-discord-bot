...
async def format_command(ctx, command):
	"""formats the command to get all the required arguments"""
	message = command[5:].strip() #remove ~join

	#parse with commas
	arguments = args = [chunk.strip() for chunk in message.split(",")] # split on commas

	if len(arguments) < 4: #if too few arguments
		args = flatten([arg.split(" ") for arg in args]) #try splitting on spaces

	if len(arguments) < 4: #if still too few arguments
